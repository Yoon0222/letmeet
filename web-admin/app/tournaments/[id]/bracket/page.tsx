...
function BracketInner() {
  ...
  return (
    <div>
      <Link href={`/tournaments/${id}`} className="text-sm text-slate-500 hover:underline">
        ← 대회 상세로
      </Link>
      <div className="mt-2">
        <h1 className="text-2xl font-semibold">{t.title} · 대진 관리</h1>
        <p className="mt-1 text-sm text-slate-500">
          {t.discipline === 'doubles' ? '복식' : '단식'} · 승인 {approved.length}
          {t.discipline === 'doubles' ? '팀' : '명'}
        </p>
      </div>

      {champion && (
        <div className="mt-4 rounded-xl bg-amber-50 p-4 text-center ring-1 ring-amber-200">
      ...
      {groupMatches.length === 0 && koMatches.length === 0 && (
        <div className="mt-4 rounded-xl border border-slate-200 bg-white p-4">
          {approved.length < 2 ? (
            <p className="text-sm text-slate-500">참가 승인이 2{t.discipline === 'doubles' ? '팀' : '명'} 이상이면 조별리그 대진을 만들 수 있어요.</p>
          ) : !isOrganizer ? (
            ...
            <div className="flex flex-wrap items-end gap-3">
              <label className="text-sm">
                <span className="mb-1 block text-slate-600">
                  조당 {t.discipline === 'doubles' ? '팀 수' : '인원'} (승인 {approved.length}{t.discipline === 'doubles' ? '팀' : '명'})
                </span>
      ...
      {t.group_count && groupMatches.length > 0 && (
        <div className="mt-4 space-y-4">
          <h2 className="text-lg font-medium">조별리그</h2>
      ...
      {groupsDone && koMatches.length === 0 && isOrganizer && (
        <div className="mt-4 flex flex-wrap items-end gap-3 rounded-xl border border-slate-200 bg-white p-4">
          ...
          <span className="pb-2 text-xs text-slate-400">
            상위 {advanceInput}{t.discipline === 'doubles' ? '팀' : '명'} 진출 → {roundName((() => { let s = 1; while (s < Math.max(2, advanceInput)) s *= 2; return s; })())} (부족분은 상위 시드 부전승)
          </span>
        </div>
      )}
      ...
      {koMatches.length > 0 && (
        <div className="mt-4 space-y-4">
          <h2 className="text-lg font-medium">본선 토너먼트</h2>
          {koRounds.map((r) => {
