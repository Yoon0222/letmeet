...
- [x] 대회 **차례 알림에 코트 포함** — notify-turn에 코트명(실내/실외) 반영, dev·prod 배포 완료 (2026-09-10). 대기번호는 미포함(보류)
...
- [x] 대회 참가비 결제 + 대기승격 결제 — **완료(0089~0092, dev 2026-09-09·prod 2026-09-10)**: 유료=결제대기 24h→결제확정, 무료=즉시확정, 대기열 승격 시 결제요청 푸시+만료 자동삭제, 조추첨 후 취소 잠금. **토스 라이브 키 전환(v3.2.3)으로 실결제 개통**
...
- **메모/주의**: dev 적용·검증(테이블·RLS 3정책, tsc/lint, 웹 부팅 무에러). **라이브 게이트 시연은 기기 재연결 시**(troy 번호 없음→게이트 뜸, 숫자라 adb 입력 가능). ⚠️ **prod 배포 시 PROD_APPLY_0093 + 앱 빌드 세트로**(테이블 없이 앱만 가면 저장 실패). 기존 유저는 다음 실행 때 1회 입력.
...
- **만든 것**: 0091(취소 잠금 트리거)·0092(코트 배정 알림 트리거) dev 적용, notify-turn 코트명 포함 재배포, web-admin courts 탭 ScoreInput+🔔, match-row readOnly 모드, 앱 tournament/[id] 탭 재구성·내 현황 카드. 전부 폰 실검증(코트 배정 푸시 "2 (실내) 코트로 배정됐어요" 수신 확인).
- **메모/주의**: prod 적용 시 0091·0092 포함 필요. 참가비 플로우 테스트 대회(12명, KDK)로 검증 — 운영 흐름: 자동배정→확정(=배정 푸시)→🔔(코트명 리마인드)→점수 저장→자동 투입.

### 안드로이드 푸시(FCM) 개통 — 백그라운드 실수신 검증 + 어드민 대회 목록 필터
- **결정**: Firebase 프로젝트 p-nut-5754c 에 com.pinut.app 등록(FCM). google-services.json 루트 + app.json `android.googleServicesFile` 연결, FCM V1 서비스계정 키는 EAS credentials 업로드(사용자). 어드민 대회 목록은 섹션 대신 **필터 칩(진행 중/종료/전체)** + [＋ 새 대회] 버튼 상단 이동.
- **만든 것**: dev 빌드 15cadd4a(FCM 포함) 설치 → troy 푸시 토큰 등록 확인 → **앱 내린 상태에서 푸시 실수신 성공**(0090 트리거→Expo→FCM 전 구간). web-admin tournaments 목록 필터 UI.
- **메모/주의**: google-services.json + app.json **커밋 필요**(비밀 아님). 운영 사용자 푸시는 **3.2.3 운영 빌드**에 포함돼야 함. 파트너 변경·차례알림 흐름의 실푸시도 이제 검증 가능. KDK 테스트 대회에 troy+테스터4명 approved 세팅(차례알림 테스트용).

## 2026-09-09

### 대회 참가비 결제 (0089·0090) — 선착순 확정 + 대기열 결제 승격 + 알림 전면 푸시화
- **결정**: 유료 대회 = 신청 시 '결제 대기'(정원 점유, 24h 마감) → 토스 결제 완료가 확정. 무료 = 신청 즉시 확정(승인 단계 제거). 대기열은 무결제 → 승격 시 24h 결제 마감+알림, 기한 만료는 자동 취소(→다음 승격). 취소는 시작 전 전액 환불. 복식 파트너는 선택 사항 — 신청 후 대진 생성 전까지 등록·변경·해제. **"모든 알림은 푸시"**: notifications AFTER INSERT 트리거가 Expo 푸시 전담(push_notify 는 저장만).
- **만든 것**: 0089(entries 결제 컬럼·트리거 확장·expire 함수·결제 우회 차단 가드) + 0090(파트너 UPDATE 중복검사·푸시 트리거) — dev 적용·SQL 전 흐름 검증(신청→대기→승격→만료+알림). toss-confirm 대회 분기(결제=확정, 만료 후 결제 복구), toss-cancel tournamentId 모드(전액 환불→행 삭제→승격). 앱: 신청 확인창 참가비 안내→바로 결제 화면(payment/court kind=tournament 재사용), 결제 대기 카드(마감 경고+결제 버튼), 파트너 카드 신청 후 모드, 정원 표시 확정+결제대기 합산. web-admin 참가자 결제 뱃지. docs/PROD_APPLY_0089.sql(+0090 포함 필요).
- **메모/주의**: ⚠️ **안드로이드 푸시 실수신 불가 상태 발견** — google-services.json 없음 → FCM 미연결 → 푸시 토큰 발급 실패(DB에 토큰 0개). Firebase 등록+EAS FCM 키+재빌드(3.2.3) 필요. 유료 결제 실완주는 토스 앱 로그인 대기(신한 일반결제는 백신앱 요구로 우회). 대회 환불 v1=시작 전 100%/후 불가.

## 2026-09-08

### 클럽 월례대회 공개 목록 제외 + 대회 탭 필터 모달(종목·연도·월)
- **결정**: 클럽 월례대회(tournaments.club_id 있음)는 공개 대회 탭·홈 추천에서 제외 — 해당 클럽 회원에게만(클럽 메뉴 경유). 내가 신청한 대회는 내 일정에 유지. 필터는 칩 나열 대신 모달로(사용자: 지역 등 필터가 늘어날 것 고려) — 상단은 [필터+개수뱃지] 요약 한 줄 + ✕해제, 모달은 종목/연도/월 섹션(데이터에 있는 값만, 연도 선택 시 그 해의 월만, 즉시 적용, 초기화/완료).
- **만든 것**: `(tabs)/tournaments.tsx` `.is('club_id', null)` + FilterSection 기반 필터 모달(새 필터는 섹션 추가만, 본문 스크롤 최대 78%), `(tabs)/index.tsx` 모집 중 대회 추천 동일 필터. dev 시드: 10월 테스트 대회·2025 송년 테스트 대회(연도/월 필터 검증용).
- **메모/주의**: 목록에서만 숨김 — 대회 상세 딥링크는 여전히 열림(RLS 공개 select). 월례대회 정식 오픈 때 상세에 클럽 멤버 가드 추가할 것. 가로 ScrollView 칩 행은 높이를 안 주면 하단이 잘림(칩 34) — 모달 전환으로 이슈 자체 제거.

### 모임 상세 탭 분리 + 클럽 메뉴 정리(경기 결과 제거·월례대회 추후 오픈)
- **결정**: 정기모임 상세를 [모임(참석)/대진 N/순위] 탭으로 분리(대진 진행 중이면 대진 탭으로 시작). 경기 결과 메뉴는 모임 상세 탭과 중복이라 삭제(`club/results.tsx` 제거), 월례대회는 추후 오픈 표시(흐림+칩, 탭 시 안내)로 진입 차단.
- **만든 것**: `club/session/[id].tsx` 탭 바, `club/[id].tsx` CLUB_MENUS(comingSoon 지원·정기모임 desc에 대진·순위·DUPR 통합 표기), `_layout.tsx` club/results 등록 제거, `sessions.tsx` 지난 모임 힌트 문구 수정.
- **메모/주의**: 지난 모임이 운영진 전용 + 경기 결과 메뉴 삭제라 일반 멤버는 지난 결과 열람 경로가 없음(당일 모임 상세에서만) — 필요해지면 지난 모임을 멤버에게 읽기전용으로 여는 걸 검토. `club/tournaments` 라우트 파일은 남아 있음(진입로만 차단, 오픈 시 칩만 제거).

### 대진 단식/복식 선택 + DUPR 인증 뱃지
- **결정**: 대진 편집에 경기 형식 토글(복식 2:2 기본/단식 1:1 — 단식은 슬롯 2개, 복식은 4명 필수 검증). 경기 시작 다이얼로그에 선수 DUPR 인증 현황 표기("⚠️ 미인증: ○○" / "✅ 전원 인증") — DUPR 모드 선택 전에 등록 가능 여부를 알 수 있게.
- **만든 것**: `club/session/match-edit.tsx`(형식 토글·검증·칩/슬롯 뱃지), `club/session/[id].tsx`(매치카드·참석모달 이름 옆 방패 뱃지, confirmStart 안내, 대진 존재 시 당일 게이트 무시하고 항상 표시 — 게이트는 자동 생성에만), `score.tsx`·`[id].tsx` 단식 이름 꼬리(" · ") 제거. 프로필 조회에 dupr_status 포함(SessionProfile 로컬 타입).
- **메모/주의**: dev 데모용으로 tester01·02·12·14 dupr_status='verified' 세팅(replica 모드로 보호 트리거 우회). 참석 모달 스크롤 이슈는 카드 감싼 Pressable 이 원인이었음 — 닫기용 Pressable 은 배경 레이어로만.

### 모임 상세 참석 현황 — 숫자 필 + 가운데 명단 모달 / 테스터 15명 시드
- **결정**: 모임 상세에 클럽원 명단이 통째로 나열되던 것 제거 — 참석/불참/미투표 숫자 필만 두고, 탭하면 가운데 모달(최대 480dp, 초과분 내부 스크롤)로 명단 표시. 운영진 액션도 모달로 이동(참석=✕제외, 미투표=+참석 추가 — 기존 '클럽원 추가' 칩 대체). 처음엔 하단 시트였으나 사용자 요청으로 가운데 다이얼로그로 변경.
- **만든 것**: `club/session/[id].tsx` listModal 상태+Modal, dev DB 테스터 15명 시드(tester01~15@peanut.test / tester!1234, 실력 2.5~4.5, 클럽 승인멤버+9/12 모임 참석 — auth.users 직접 insert, `scratchpad/seed_testers.sql`).
- **메모/주의**: 아메리카노 자동 생성은 참석 4명 이상 + 모임 당일 0시부터만. 시드 중 관리 API 504 간헐 — 재시도로 해결.

### 클럽 경기 결과 = 정기모임 대진 결과로 일원화 + 지난 모임 운영진 전용
- **결정**: 클럽 '경기 결과'가 별도 수동 기록(`club_match_results`+match-create)으로 분리돼 있던 걸 제거 — 정기모임 대진(`club_session_matches`)의 완료 경기만 세션별로 묶어 보여준다(사용자: "대진에 대한 경기결과만 표시하면 되는거야"). DUPR 등록은 기존 `source:'club_session'` 재사용이라 서버 변경 없음.
- **만든 것**: `club/results.tsx` 재작성(세션 그룹 헤더→매치 카드·DUPR/일반 칩·관리자 DUPR 등록 버튼), `club/match-create.tsx` 삭제, `club/sessions.tsx` 예정/지난 모임 섹션 분리 + 지난 모임은 운영진에게만 표시(멤버는 경기 결과에서 확인), `club/[id].tsx` 메뉴 순서 정기모임→경기 결과로 인접 배치. 폰 라이브 검증: 매치 추가→DUPR 모드 시작→16:8 저장→경기 결과 화면 그룹 표시까지 완주.
- **메모/주의**: 대진 자동 생성(아메리카노)은 참석 4명 미만이면 인원 부족 알림 — 2명 테스트는 '매치 추가'로. `club_match_results` 테이블·타입은 DB에 남아 있음(과거 데이터, 화면에서만 미사용).

### 토스 결제화면 2건 수정 — 상태바 자동숨김 + 나가기 경로
- **결정**: "카드 등록 창이 준비되었습니다" 상태바가 결제창 위에 상시 떠 있던 것(onLoadEnd마다 세팅, 숨김 로직 없음)을 "보여줄 게 있을 때만"으로 변경. 헤더 없는 전체화면(`payment/subscribe`·`payment/court`)에 나가는 길이 없어 우상단 ✕(취소 확인 다이얼로그) 추가, 안드로이드 뒤로가기도 같은 확인 경로.
- **만든 것**: `payment/subscribe.tsx`·`payment/court.tsx` — status nullable+onLoadEnd 클리어, 확정/승인 중 스피너, ✕ 버튼(court는 주문 취소까지), confirmLeave. 클럽 `match-create` 화면의 선수 칩 0폭 붕괴(flexBasis 부재)·저장 버튼 내비바 겹침도 고쳤으나 위 일원화로 파일 자체 삭제됨.
- **메모/주의**: 3.2.2 빌드에는 미포함 — 토스 결제경로 심사 캡처는 dev 앱에서 찍을 것. 다음 스토어 빌드(3.2.3)에 포함.

## 2026-09-02

### v3.2.0 릴리스 — 클럽 게시판·홈 공지 포함, prod 빌드
- **결정**: 3.1.2 이후 누적분(DUPR+ 게이팅·수정삭제 요청·종목·매치 탭·클럽 게시판·홈 공지)을 3.2.0으로 태그. EAS 첫 시도는 서버 INTERNAL_SERVER_ERROR — 재시도로 진행.
- **만든 것**: 클럽 게시판(0088 — 클럽원 전용 글/댓글+공지, `club/board`·`club/post/[id]`, 상세 메뉴·홈 공지 섹션), v3.2.0 태그(`1fbcd16`), web-admin Vercel 배포.
- **메모/주의**: ⚠️ **prod DB 0084~0088 미실행 상태로 스토어 배포 금지** — match_count 뷰 없으면 번개 목록이 통째로 빈다. 노션 릴리스 노트는 커넥터 부재로 이 세션에서 못 남김(수동 반영 필요).
...
- **메모**: 평가 규칙 3계층 동일(알고리즘 10/10 검증). **dev 적용·함수배포·컬럼검증 완료**. prod = `docs/PROD_APPLY_0076.sql` 실행 후 `toss-cancel` prod 배포(순서 중요). 후속: 부분환불 시 payments.status는 'refunded'로만(부분 상태 별도 없음).
