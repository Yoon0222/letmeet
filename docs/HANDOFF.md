...
**단체전 1~5 전 기능 구현·검증 완료.** 알려진 마이너: 오더 저장 후 대진뷰(TeamBracketView) 자동 새로고침 안 됨(재진입/새로고침 시 반영) — TeamLineup/TeamBracketView 가 각자 로드하는 형제 컴포넌트라 공유 reload 없음. 개선여지: 오더 동시제출·블라인드 공개.

Follow-ups (마이그레이션):
- 개발 DB: 0036~0040 전부 실행됨.
