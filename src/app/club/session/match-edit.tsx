...
// 단식이면 각 팀 1번 슬롯만 쓴다
const SINGLES_LABEL: Partial<Record<SlotKey, string>> = { t1p1: 'A 선수', t2p1: 'B 선수' };
...
export default function MatchEdit() {
  ...
  const [attendees, setAttendees] = useState<SessionProfile[]>([]);
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const { data: pl } = await supabase
        ...
        .select('user_id, profiles(id, nickname, skill_level, avatar_url, region, dupr_status)')
        .eq('session_id', sessionId)
        .eq('status', 'in');
      const list = ((pl as unknown as { profiles: SessionProfile | null }[]) ?? []).map((r) => r.profiles).filter(Boolean) as SessionProfile[];
      ...
      if (matchId) {
        ...
        if (match) {
          ...
          setFormat(match.team1_player2 || match.team2_player2 ? 'doubles' : 'singles');
  ...
  // 단식은 각 팀 1번 슬롯만 사용
  const visibleSlots = useMemo(() => (format === 'singles' ? SLOTS.filter((s) => s.key === 't1p1' || s.key === 't2p1') : SLOTS), [format]);
  ...
  function changeFormat(next: MatchFormat) {
    ...
    if (next === 'singles') {
      // 파트너 슬롯 비우기
      setSlots((prev) => ({ ...prev, t1p2: null, t2p2: null }));
      setActive((prev) => (prev === 't1p2' || prev === 't2p2' ? 't1p1' : prev));
    }
  }
  ...
  function assign(pid: string) {
    ...
    // 다음 빈 슬롯으로 이동 (현재 형식의 슬롯만)
    const order = visibleSlots.map((s) => s.key);
  ...
  async function save() {
    ...
    if (!slots.t1p1 || !slots.t2p1) {
      Alert.alert('선수 확인', format === 'singles' ? 'A·B 선수를 모두 배정해야 해요.' : '각 팀에 최소 1명(A팀 선수 1, B팀 선수 1)은 배정해야 해요.');
      return;
    }
    if (format === 'doubles' && (!slots.t1p2 || !slots.t2p2)) {
      Alert.alert('선수 확인', '복식은 각 팀 2명씩 배정해야 해요. 2명이서 치는 경기면 단식을 선택하세요.');
      return;
    }
    setSaving(true);
    const payload = {
      ...
      team1_player2: format === 'singles' ? null : slots.t1p2,
      team2_player1: slots.t2p1,
      team2_player2: format === 'singles' ? null : slots.t2p2,
    };
    const { error } = editing
    ...
    <ScrollView style={styles.flex} contentContainerStyle={styles.content}>
      <View style={styles.field}>
        ...
        <View style={styles.segRow}>
          <Pressable onPress={() => changeFormat('doubles')} style={[styles.segBtn, format === 'doubles' && styles.segOn]}>
            <Ionicons name="people" size={15} color={format === 'doubles' ? '#07100D' : '#AAB4C0'} />
            <Text style={[styles.segTxt, format === 'doubles' && styles.segTxtOn]}>복식 (2:2)</Text>
          </Pressable>
          <Pressable onPress={() => changeFormat('singles')} style={[styles.segBtn, format === 'singles' && styles.segOn]}>
            <Ionicons name="person" size={15} color={format === 'singles' ? '#07100D' : '#AAB4C0'} />
            <Text style={[styles.segTxt, format === 'singles' && styles.segTxtOn]}>단식 (1:1)</Text>
          </Pressable>
        </View>
      </View>
      ...
      <View style={styles.slotGrid}>
        {visibleSlots.map((s) => (
          <Pressable key={s.key} onPress={() => setActive(s.key)} style={[styles.slot, active === s.key && styles.slotActive, s.team === 2 && styles.slotTeam2]}>
            <Text style={styles.slotLabel}>{format === 'singles' ? SINGLES_LABEL[s.key] ?? s.label : s.label}</Text>
            <Text style={[styles.slotName, !slots[s.key] && styles.slotEmpty]} numberOfLines={1}>
              {nameOf(slots[s.key])}
              {duprOk(slots[s.key]) ? <Ionicons name="shield-checkmark" size={12} color="#60A5FA" /> : null}
            </Text>
            {slots[s.key] ? (
        ...
        {attendees.map((a) => {
          ...
          return (
            <Pressable key={a.id} onPress={() => assign(a.id)} style={[styles.playerChip, used && styles.playerChipUsed]}>
              ...
              {a.dupr_status === 'verified' ? <Ionicons name="shield-checkmark" size={12} color="#60A5FA" /> : null}
            </Pressable>
          );
        })}
...
const styles = StyleSheet.create({
  ...
  segBtn: { flex: 1, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 6, minHeight: 46, borderRadius: 14, borderCurve: 'continuous', backgroundColor: '#10161D', borderWidth: 1, borderColor: 'rgba(255,255,255,0.09)' },
  segOn: { backgroundColor: '#16C784', borderColor: '#16C784' },
  segTxt: { color: '#AAB4C0', fontSize: 14, fontWeight: '800' },
  segTxtOn: { color: '#07100D' },
