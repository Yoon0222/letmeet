...
export default function CourtMap({ courts, onSelect }: CourtMapProps) {
  const pts = courts.filter((c) => c.latitude != null && c.longitude != null);
  const center = pts.length
    ? { latitude: avg(pts.map((p) => p.latitude as number)), longitude: avg(pts.map((p) => p.longitude as number)) }
    : SEOUL;
